...
def test_fmt_service_compare_write_returns_candidates_for_multiple_strategies() -> None:
    session = build_session()
    session.rec.set_context("arch.bits", 64, domain=RecordDomain.FMT)
    session.rec.set_context("arch.endian", "little", domain=RecordDomain.FMT)
    session.fmt.set_offset(6)

    result = session.fmt.compare_write(0x6010A0, 0x601018)

    assert isinstance(result, FmtWriteComparison)
    ...
    assert all(item.ok for item in result.candidates)
    assert result.candidates[0].rendered_tasks[0].payload
    text = str(result)
    assert "[FMT] target=0x6010a0 value=0x601018" in text
    assert "◆ AUTO" in text
    assert "◆ BYTE" in text
    assert "❔" in text
    assert "max_pad 4024" in text
    assert "pad_time MEDIUM" in text
    assert "task[0]" not in text


def test_fmt_service_compare_write_accepts_string_strategies() -> None:
    session = build_session()
    session.rec.set_context("arch.bits", 64, domain=RecordDomain.FMT)
    session.rec.set_context("arch.endian", "little", domain=RecordDomain.FMT)
...
def test_fmt_service_compare_write_uses_buflen_status_icons() -> None:
    session = build_session()
    session.rec.set_context("arch.bits", 64, domain=RecordDomain.FMT)
    session.rec.set_context("arch.endian", "little", domain=RecordDomain.FMT)

    ok_result = session.fmt.compare_write(
        0x6010A0,
        0x601018,
        offset=6,
        buflen=64,
    ...
    bad_result = session.fmt.compare_write(
        0x6010A0,
        0x601018,
        offset=6,
        buflen=16,
...
def test_fmt_service_compare_write_captures_strategy_failures() -> None:
    ...
    assert "FmtDataOffsetResolutionError" in (result.candidates[0].error or "")
...
def test_default_renderer_uses_pwntools_fmt_and_data_split() -> None:
    ...
    assert rendered.payload.startswith(rendered.fmt_bytes)
    assert rendered.payload.endswith(rendered.data_bytes)
...
def test_pwntools_backend_render_matches_fmtstr_payload_ground_truth() -> None:
    ...
    assert rendered.backend == "pwntools"
    assert rendered.payload == expected_payload
    assert rendered.data_offset == 12
    assert [step.arg_index for step in rendered.steps] == [12, 13, 14, 15]
...
def test_pwntools_backend_computes_task_local_data_offset_on_32bit() -> None:
    session = build_session()
    session.rec.set_context("arch.bits", 32, domain=RecordDomain.FMT)
    session.rec.set_context("arch.endian", "little", domain=RecordDomain.FMT)
    ...
    assert rendered.steps[0].arg_index == 10


def test_pwntools_backend_recomputes_data_offset_for_by_atom_tasks() -> None:
    session = build_session()
    plan = session.fmt.plan_writes(
        {0x404018: 0x11223344},
        strategy=FmtWriteStrategy.SHORT,
    ...
    rendered_items = session.fmt.render_plan(plan, offset=6)

    assert len(rendered_items) == plan.total_tasks == 2
    assert [item.data_offset for item in rendered_items] == [8, 8]
    assert [[step.arg_index for step in item.steps] for item in rendered_items] == [[8], [8]]
    assert [item.metadata["stabilization_steps"] for item in rendered_items] == [2, 2]


def test_pwntools_backend_uses_task_level_data_offset_for_by_target_tasks() -> None:
    session = build_session()
    plan = session.fmt.plan_writes(
        {0x404018: 0x11223344, 0x404020: 0x55667788},
        strategy=FmtWriteStrategy.SHORT,
    ...
    assert [item.data_offset for item in rendered_items] == [9, 9]
    assert [[step.arg_index for step in item.steps] for item in rendered_items] == [
        [9, 10],
        [9, 10],
    ]


def test_pwntools_backend_rejects_explicit_data_offset_that_is_too_small() -> None:
    session = build_session()
    plan = session.fmt.plan_writes(
        {0x60120: 0x66},
        strategy=FmtWriteStrategy.BYTE,
    ...
    with pytest.raises(FmtDataOffsetResolutionError):
        session.fmt.render_task(plan.tasks[0], plan=plan, offset=6, data_offset=6)
...
def test_default_executor_supports_explicit_end_suffix() -> None:
    session = build_exec_session(b"done\n")
    atom = FmtWriteAtom(
        request_index=0,
        piece_index=0,
    ...
    assert receipt.dispatch == FmtExecutionMethod.SEND
    assert session.transport.sent == [("send", rendered.payload + b" ")]  # type: ignore[attr-defined]
